...
### RustScan Alternative (TCP Only)

[RustScan](https://github.com/RustScan/RustScan) is a fast port scanner that can be used as an alternative to nmap for TCP scanning through Mutiauk:
...
:::warning RustScan: TCP Only
RustScan only supports TCP scanning. It cannot perform UDP scans because:

1. **Design limitation**: RustScan is built specifically for fast TCP port discovery using asynchronous TCP connect calls
2. **No UDP implementation**: RustScan does not implement UDP socket handling or the timeout-based detection that UDP scanning requires
3. **SOCKS5 compatibility**: While TCP connections work transparently through SOCKS5/TUN, RustScan has no mechanism to utilize SOCKS5 UDP ASSOCIATE

For UDP scanning, use nmap with the `-sU` flag instead.
:::
